# By Ada Pluguez
# 4/1/24
# YouTube Title/Description Generator Client Application
...
# Placeholder for an in-memory key database
key_database = {}
...
# Load an encryption key from a file, or generate a new one if it doesn't exist
def load_key():
...
class UserRegistrationForm(simpledialog.Dialog):
    ...
    def apply(self):
        self.result = {
            ...
            "regkey": self.regkey.get()
        }
  
class UserLoginForm(simpledialog.Dialog):
    def body(self, master):
        tk.Label(master, text="Username:").grid(row=0)
        tk.Label(master, text="Password:").grid(row=1)
...
def register_or_verify_user(master, is_new_user):
    if is_new_user:
        form = UserRegistrationForm(master, title="New User Setup")
        if form.result:
            username = form.result['username']
            password = form.result['password']
            email = form.result['email']
            regkey = form.result['regkey']
            
            if not is_password_valid(password):
                messagebox.showerror("Invalid Password", "Password must be at least 8 characters, include an uppercase letter, and a symbol.")
            ...
            if response.status_code == 201:
                messagebox.showinfo("Registration", "Registration successful. Logging you in...")
                login_response = requests.post(f"{SERVER_URL}/login", json={"username": username, "password": password})
                if login_response.status_code == 200:
                    messagebox.showinfo("Login", "Login successful.")
                    return True, "Login successful.", username
                else:
                    try:
                        error_message = login_response.json().get('message', 'Login failed after registration.')
                    except json.JSONDecodeError:
                        error_message = "Invalid server response. Please contact support."
                    messagebox.showerror("Login Error", error_message)
                    return False, "Login failed after registration.", None
            else:
                try:
                    error_message = response.json().get('message', 'Unknown error during registration.')
                except json.JSONDecodeError:
                    error_message = "Invalid server response during registration. Please contact support."
                messagebox.showerror("Registration Error", error_message)
                return False, "Registration failed.", None
        else:
            return False, "Registration cancelled.", None
    else:
        form = UserLoginForm(master, title="Login")
        ...
        if form_result:
            ...
            if response.status_code == 200:
                ...
                try:
                    error_message = response.json().get('message', 'Verification failed. Please check your credentials.')
                except json.JSONDecodeError:
                    error_message = "Invalid server response during verification. Please contact support."
                messagebox.showerror("Login Error", error_message)
                return False, "Login failed.", None
        else:
            return False, "Login cancelled.", None

# Encrypt and save API key and client secrets path to a file
...
# Load and decrypt API key and client secrets path from a file
...
class YoutubeTitleDescriptionGeneratorGUI:
    def __init__(self, master):
        self.master = master
        master.title("YouTube Title/Description Generator Generator")
        master.protocol("WM_DELETE_WINDOW", self.on_closing)  # Bind the closing event


        # Start by verifying the user
        self.user_verification()
        
        # Load or prompt for API and client secrets after verification
        self.api_key, self.client_secrets_file = get_credentials()
        
        # If API Key or Client Secrets File is not provided, exit the application
        ...
        # Fetch the total number of videos for content management
        ...
        # Input fields for video titles and descriptions
        ...
        # Output fields to display generated content
    ...
    def generate_content(self):
        # Extract and process video title and description from user inputs
    ...
    def user_verification(self):
        while True:
            ...
            success, message, username = register_or_verify_user(self.master, is_new_user)
            if success:
                self.username = username  # Set the username attribute here
                break
            else:
    ...
    def on_closing(self):
        if messagebox.askokcancel("Quit", "Do you really wish to quit?"):
            try:
                ...
                # Optionally force quit if normal quit fails
                self.master.quit()

    def logout_user(self):
        if hasattr(self, 'username'):
            response = requests.post(f"{SERVER_URL}/logout", json={"username": self.username})
            if response.status_code == 200:
                messagebox.showinfo("Logout", "Logged out successfully.")
            else:
                messagebox.showerror("Logout Failed", "Could not log out.")
        else:
            messagebox.showinfo("Info", "No user is logged in.")

   
def main():
    root = tk.Tk()
    gui = YoutubeTitleDescriptionGeneratorGUI(root)
