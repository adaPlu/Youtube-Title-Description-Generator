...
class YoutubeContentGeneratorGUI:
    def __init__(self, master):
        ...
        # Show an information messagebox as a workaround
        messagebox.showinfo("API Key,", "If first run or API key not found, you must enter your OpenAI API key:")
    ...
    def verify_user_access(self):
        """Prompts the user for a User ID and Key, returning True if verification is successful."""
        user_id = simpledialog.askstring("User ID", "Enter your User ID:", parent=self.master)
        user_key = simpledialog.askstring("Key", "Enter your Key:", parent=self.master)
        if user_id and user_key:
            return verify_key(user_id, user_key)  # Assuming verify_key returns True if the key is valid
        return False
